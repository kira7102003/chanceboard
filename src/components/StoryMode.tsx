...
export default function StoryMode({onClose,onComplete,onBattle}:Props){
 ...
 const enter=(item:StoryChapter)=>{if(!item.unlocked)return;const flow=getChapterFlow(item),picker=flow.find((node):node is Extract<StoryFlowNode,{type:'branch'}>=>node.type==='branch'&&!!node.chapterRouteSelect),route=picker?.branches.find(branch=>branch.id===routeChoice?.id||branch.label===routeChoice?.label);setChapter(item);setNodes(picker&&route?flow.flatMap(node=>node.id===picker.id?route.nodes:[node]):flow);setEntryRoute(route?{id:route.id,total:route.nodes.length}:undefined)}
