...
export default function StoryMode({ onClose }: Props) {
  ...
  const lines = chapter?.story.split('\n').map(text => text.trim()).filter(Boolean) ?? []

  if (chapter) {
    const text = lines[line] ?? '【章節結束】'
    return <div className="story-stage" style={{ backgroundImage: `linear-gradient(180deg,rgba(2,3,12,.2),#03040e 95%),url(${getUrlByKey(`cb_story_map_${chapter.id}`) ?? ''})` }}>
      ...
      </div> : <button className="story-dialogue" onClick={() => setLine(value => Math.min(lines.length, value + 1))}>
        <span>{route === 'black' ? '小黑' : route === 'white' ? '小白' : chapter.title}</span>
        <p>{text}</p><small>{line >= lines.length ? '點擊返回章節地圖' : '點擊繼續　▶'}</small>
      </button>}
      {line >= lines.length && <button className="btn primary story-finish" onClick={() => { setChapter(null); setLine(0); setRoute(null) }}>完成章節</button>}
    </div>
  }
