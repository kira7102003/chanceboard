...
export default function StoryMode({onClose,onComplete,onBattle}:Props){
 ...
 const mapPoints=chapters.map((item,index)=>({x:item.mapX??[9.5,28.5,45.5,62.5,80.5,93][index],y:item.mapY??[68,41,64,33,57,23][index]})),selectedIndex=Math.max(0,chapters.findIndex(item=>item.id===selectedId)),actorId=player.desktopCharIds?.[0]??player.ownedCharIds[0]
 ...
 return <div className="panel-overlay story-map-screen">
  ...
  <div className="story-world-map">
   ...
   <div className="story-map-route">
    ...
    <svg className="story-route-line" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true"><polyline points={mapPoints.map(point=>`${point.x},${point.y}`).join(' ')}/></svg>
    ...
    {chapters.map((item,index)=> <button key={item.id} aria-disabled={!item.unlocked} style={{left:`${mapPoints[index].x}%`,top:`${mapPoints[index].y}%`}} className={`story-map-node ${item.unlocked?'unlocked':'locked'} ${selectedId===item.id?'selected':''} node-${index+1}`} onClick={()=>setSelectedId(item.id)} onDoubleClick={()=>enter(item)}>
