...
export default function StoryMode({onClose,onComplete,onBattle}:Props){
 ...
 if(showEntryPicker&&entryPicker)return <StoryRoutePicker chapter={entryChapter} node={entryPicker} onBack={onClose} onChoose={route=>{setRouteChoice({id:route.id,label:route.label});setShowEntryPicker(false)}}/>
 ...
 const enter=(item:StoryChapter)=>{if(!item.unlocked)return;const flow=getChapterFlow(item),picker=flow.find((node):node is Extract<StoryFlowNode,{type:'branch'}>=>node.type==='branch'&&!!node.chapterRouteSelect),route=picker?.branches.find(branch=>branch.id===routeChoice?.id||branch.label===routeChoice?.label),routeFlow=picker&&route?flow.flatMap(node=>node.id===picker.id?route.nodes:[node]):flow,hasOpeningCard=routeFlow[0]?.type==='common'&&routeFlow[0].segment.presentation==='chapter',chapterCard:StoryFlowNode={id:`chapter_card_${item.id}`,type:'common',segment:{id:`chapter_card_segment_${item.id}`,speaker:'旁白',side:'left',presentation:'chapter',section:item.chapterCardEyebrow??'CHAPTER',text:item.chapterCardTitle??`第${item.order}章　${item.piece}`,chapterPrompt:item.chapterCardPrompt??'點擊任意位置開始 ◆'}};setChapter(item);setNodes(item.chapterCardEnabled===false||hasOpeningCard?routeFlow:[chapterCard,...routeFlow]);setEntryRoute(route?{id:route.id,total:route.nodes.length}:undefined)}
