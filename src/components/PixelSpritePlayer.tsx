...
export default function PixelSpritePlayer({src,fps,adjustments,playing=true,selectedFrame=0}:{src:string;fps:number;adjustments:FrameAdjust[];playing?:boolean;selectedFrame?:number}){
  const frame=Math.max(0,Math.min(3,selectedFrame))
  if(!playing){const item=adjustments[frame]??DEFAULT_ADJUST;return <div className="pixel-sprite-player"><div className="pixel-sprite-layer pixel-sprite-static" style={{backgroundImage:`url(${src})`,backgroundPosition:positions[frame],transform:`translate(${item.x}%,${item.y}%) scale(${item.scale/100})`}}/></div>}
