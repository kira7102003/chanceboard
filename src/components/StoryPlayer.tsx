...
export default function StoryPlayer({chapter,initialNodes,onLeave,onComplete,onBattle,preview=false}:Props){
 ...
 const advance=()=>{if(!complete){setShown(revealMaximum);return}addHistory();setCursor(value=>Math.min(nodes.length,value+1))}
