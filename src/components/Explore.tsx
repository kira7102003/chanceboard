...
export default function Explore({ onClose }: Props) {
  ...
  const attackRock = (charId: string, moveId?: string) => {
    ...
    if(!char||!node||!object||manualFinished||Date.now()>=manualEndAt||manualHp[target]<=0||attacking||(skillCooldowns[cooldownKey]??0)>Date.now())return
  ...
  const moveOnGrid=(event:React.MouseEvent<HTMLElement>)=>{if(manualFinished||Date.now()>=manualEndAt||!activeMiner||(event.target as HTMLElement).closest('button'))return;const rect=event.currentTarget.getBoundingClientRect(),x=Math.round((event.clientX-rect.left)/rect.width*49)/49*100,y=Math.round((event.clientY-rect.top)/rect.height*49)/49*100;setManualPositions(points=>points.map((point,index)=>selected[index]===activeMiner?{x,y}:point))}
  if (manual) return <div className="panel-overlay mining-manual">
    ...
    <section className="manual-mine-grid" onClick={moveOnGrid}><div className="manual-tile-field" aria-label="50×50 礦坑地磚"><i className="manual-target-tile" style={{left:`${config.nodes[target]?.x??50}%`,top:`${config.nodes[target]?.y??50}%`}}/></div>
      ...
      {selected.map((id,index)=><div className={`manual-worker active ${manualFinished?'finished':''} ${attacking.startsWith(id)?'attacking':''}`} key={id} style={{left:`${manualPositions[index]?.x??50}%`,top:`${manualPositions[index]?.y??80}%`}}><PixelCharacterActor charId={id} pose="side" action={manualFinished?'idle':'mining'} face="right"/><b>{chars.find(char=>char.id===id)?.name}</b></div>)}
