...
export default function Explore({ onClose }: Props) {
  ...
  if (manual) return <div className="panel-overlay mining-manual"><div className="panel-header"><button className="panel-back" onClick={()=>setManual(false)}>返回挖礦</button><span className="panel-title">⛏ 手動挖掘 · 50×50</span><span className="manual-combo">{combo>1?`${combo} COMBO · +${(combo-1)*10}%`:activeMiner?`${chars.find(char=>char.id===activeMiner)?.name} 操作中`:''}</span><span className={`panel-meta manual-timer ${manualSeconds<=5?'danger':''}`}>剩餘 {manualSeconds}s</span></div><section className="manual-mine-grid" onClick={moveOnGrid}><div className="manual-tile-field" aria-label="50×50 礦坑地磚"><i className="manual-target-tile" style={{left:`${config.nodes[target]?.x??50}%`,top:`${config.nodes[target]?.y??50}%`}}/></div>{manualDebris.map(item=><i key={item.id} className={`manual-debris ${item.kind}`} style={{left:`${item.x}%`,top:`${item.y}%`}}/>)}{config.nodes.map((node,index)=><button key={node.id} disabled={manualFinished||manualHp[index]<=0} className={`manual-rock ${target===index?'selected':''} ${manualHp[index]<=0?'cleared':''}`} style={{left:`${node.x}%`,top:`${node.y}%`}} onClick={()=>setTarget(index)}><span>{manualHp[index]<=0?'✓':'◆'}</span><b>{node.name}</b><strong>{rewardLabel[node.reward]??node.reward}</strong><i><em style={{width:`${manualHp[index]/node.maxHp*100}%`}}/></i><small>{Math.ceil(manualHp[index]).toLocaleString()} / {node.maxHp.toLocaleString()}</small></button>)}{selected.map((id,index)=><div className={`manual-worker ${activeMiner===id?'active':''} ${attacking.startsWith(id)?'attacking':''}`} onClick={event=>{event.stopPropagation();setActiveMiner(id)}} key={id} style={{left:`${manualPositions[index]?.x??50}%`,top:`${manualPositions[index]?.y??80}%`}}><PixelCharacterActor charId={id} pose="side" action={attacking.startsWith(id)?'mining':'walk'} face="right"/><b>{chars.find(char=>char.id===id)?.name}</b></div>)}{manualHp.filter(hp=>hp<=0).length>=2&&!manualFinished&&<button className="manual-stairs" onClick={()=>{setMessage('找到下一層！本層挖掘完成');setManualEndAt(Date.now())}}>▼<b>下一層</b></button>}{manualFinished&&<div className="manual-result"><b>{manualHp.every(hp=>hp<=0)?'全部礦物清除！':'本層結束'}</b><span>已清除 {manualHp.filter(hp=>hp<=0).length} / {manualHp.length} 個挖掘點</span><button onClick={enterManual}>進入下一層</button></div>}</section><div className="manual-skills">{selected.flatMap(id=>moves.filter(move=>move.ownerId===id&&move.slot!=='被').map(move=><button disabled={manualFinished||!!attacking||manualHp[target]<=0} className={`${activeMiner===id?'active-miner':''} ${attacking===`${id}_${move.id}`?'attacking':''}`} key={`${id}_${move.id}`} onClick={()=>attackRock(id,move.id)}>{getMoveImg(move.id)&&<img src={getMoveImg(move.id)!} alt=""/>}<span>{chars.find(char=>char.id===id)?.name}<b>{move.name}</b><small>礦物傷害 {Math.round((chars.find(char=>char.id===id)?.atk??1)*(move.powerRatio??1)*120)}</small></span></button>))}</div>{message&&<output className="manual-message">{message}</output>}</div>
