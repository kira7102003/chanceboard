...
export default function Explore({ onClose }: Props) {
  ...
  return <div className="panel-overlay mining-screen" data-daily-tick={dailyTick}><div className="panel-header"><button className="panel-back" onClick={onClose}>返回大廳</button><span className="panel-title"><FunctionIcon name="explore"/>礦坑探索</span><span className="panel-meta">每日 04:00 重置</span></div><main className="mining-layout"><section className="mining-map">{nodeState.map((state,index)=><div className={`mining-node mining-node-${index+1} ${state.percent<40?'cracked':''}`} style={{left:`${state.x}%`,top:`${state.y}%`}} key={state.id}><div className="mining-hp"><b>{state.name}</b><span>{Math.ceil(state.hp).toLocaleString()} / {state.maxHp.toLocaleString()}</span><i><em style={{width:`${state.percent}%`}}/></i><small>已挖碎 {state.broken} 塊 · 今日 {state.uses}/3</small></div>{state.char&&<div className="mining-worker"><PixelCharacterActor charId={state.char.id} pose="side" action="walk" face={index===2?'left':'right'}/><label>{state.char.name}<small>力度 {state.stats?.power.toFixed(1)} · 速度 +{state.stats?.speed.toFixed(0)}%</small></label></div>}<div className="mining-node-action">{!state.job?<button disabled={state.uses>=3} onClick={()=>startNode(state.id)}>自動挖掘</button>:state.finished?<button className="ready" onClick={()=>claimNode(state.id)}>領取</button>:<><span>{fmt((state.job.endsAt-now)/1000)}</span><button onClick={()=>cancelNode(state.id)}>取消</button></>}</div></div>)}</section><aside className="mining-side"><h2>選擇挖礦角色</h2><p>一個礦點只能一人；同一角色不能重複派遣。</p><div className="mining-roster">{available.map(char=>{const deployed=run.jobs.some(job=>job.charId===char.id);return <button disabled={deployed} className={selected.includes(char.id)?'selected':''} onClick={()=>setSelected(ids=>ids.includes(char.id)?ids.filter(id=>id!==char.id):ids.length<3?[...ids,char.id]:ids)} key={char.id}>{getCharImg(char.id)&&<img src={getCharImg(char.id)!} alt=""/>}<span>{char.name}<small>{deployed?'自動挖掘中':`ATK ${char.atk} · SPD ${char.spd}`}</small></span></button>})}</div><button className="mining-main manual" onClick={enterManual}>進入 50×50 手動挖掘</button>{message&&<output>{message}</output>}</aside></main></div>
}
