...
export default function Explore({ onClose }: Props) {
  ...
  const attackRock = (charId: string, moveId?: string) => {
    const char=chars.find(item=>item.id===charId),move=moveId?moves.find(item=>item.id===moveId):undefined,node=config.nodes[target],object=manualDebris.find(item=>item.id===selectedDebris&&item.kind!=='pit')
    ...
    window.setTimeout(()=>{setDigMarks(marks=>[...marks.slice(-23),{id:Date.now(),x:object.x,y:object.y,size:18+Math.random()*24}]);setManualDebris(items=>items.filter(item=>item.id!==object.id));setSelectedDebris(null);setManualHp(current=>{if(Date.now()>=manualEndAt)return current;const next=[...current],before=next[target];next[target]=Math.max(0,before-damage);if(before>0&&next[target]===0){const rewards=rollMiningRewards(node),summary=Object.entries(rewards).map(([key,amount])=>`${rewardLabel[key]??key} +${amount}`).join('、')||'沒有額外掉落';player.addResourceRewards(rewards);setMessage(`${node.name}已清除：${summary}`)}return next});setAttacking('')},420)
  }
