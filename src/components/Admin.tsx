...
function StoryMapRouteEditor({chapters,onMove,onRouteChange}:{chapters:StoryChapter[];onMove:(index:number,x:number,y:number)=>void;onRouteChange:(points:{x:number;y:number}[])=>void}){
  ...
  const addPoint=()=>{let segment=0,length=-1;routePoints.slice(0,-1).forEach((point,index)=>{const next=routePoints[index+1],value=(next.x-point.x)**2+(next.y-point.y)**2;if(value>length){length=value;segment=index}});const from=routePoints[segment]??{x:40,y:50},to=routePoints[segment+1]??{x:60,y:50},next=[...routePoints.slice(0,segment+1),{x:(from.x+to.x)/2,y:(from.y+to.y)/2},...routePoints.slice(segment+1)];onRouteChange(next);setSelectedPoint(segment+1)}
