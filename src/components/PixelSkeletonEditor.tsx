...
export default function PixelSkeletonEditor({ charId }: { charId: string }) {
  const [mode, setMode] = useState<'frames' | 'skeleton'>('frames')
  ...
  return <div className="pixel-skeleton-editor">
    <div className="pixel-animation-mode"><button className={mode==='frames'?'active':''} onClick={()=>setMode('frames')}>逐格動畫（建議）</button><button className={mode==='skeleton'?'active':''} onClick={()=>setMode('skeleton')}>拆件骨架</button></div>
    {mode==='frames'?<PixelFrameAnimator charId={charId} fallback={image}/>:<>
