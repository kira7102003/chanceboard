...
import PixelSpriteSheetEditor from './PixelSpriteSheetEditor'
...
export default function PixelSkeletonEditor({ charId }: { charId: string }) {
  const [mode, setMode] = useState<'sheet' | 'motion' | 'frames' | 'skeleton'>('sheet')
  ...
  return <div className="pixel-skeleton-editor">
    <div className="pixel-animation-mode"><button className={mode==='sheet'?'active':''} onClick={()=>setMode('sheet')}>2×2 動畫圖（建議）</button><button className={mode==='motion'?'active':''} onClick={()=>setMode('motion')}>整張微動</button><button className={mode==='frames'?'active':''} onClick={()=>setMode('frames')}>逐格圖片</button><button className={mode==='skeleton'?'active':''} onClick={()=>setMode('skeleton')}>拆件骨架</button></div>
    {mode==='sheet'?<PixelSpriteSheetEditor charId={charId}/>:mode==='motion'?<div className="pixel-whole-motion"><div className="pixel-frame-actions">{(['idle','walk','mining'] as Action[]).map(value=><button key={value} className={action===value?'active':''} onClick={()=>setAction(value)}>{value==='idle'?'待機':value==='walk'?'走路':'挖礦'}</button>)}</div><div className={`pixel-whole-preview whole-${action}`}>{image?<img src={image} alt="整張角色微動預覽"/>:<span>尚未上傳角色圖片</span>}{action==='mining'&&<><b>⛏</b><i/></>}</div><small>整張圖片保持完整，只做小幅位移與傾斜，因此不會出現關節裂縫。</small></div>:mode==='frames'?<PixelFrameAnimator charId={charId} fallback={image}/>:<>
