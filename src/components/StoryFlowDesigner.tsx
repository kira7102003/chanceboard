...
function FlowGraphOverview({chapter,nodes,onChange,onChapterChange,onLocate,open,onOpenChange}:{chapter:StoryChapter;nodes:StoryFlowNode[];onChange:(nodes:StoryFlowNode[])=>void;onChapterChange:(patch:Partial<StoryChapter>)=>void;onLocate:(id:string)=>void;open:boolean;onOpenChange:(open:boolean)=>void}){
  ...
  const[manualSource,setManualSource]=useState('__world_map__'),[manualTarget,setManualTarget]=useState('__map_chapter__'),[manualLabel,setManualLabel]=useState('下一步')
  ...
  const linkNodes=[{id:'__world_map__',name:'候選地圖'},{id:'__map_chapter__',name:`地圖關卡｜${chapter.mapNodeTitle??chapter.title}`},...(entryPicker?[{id:'__route_picker__',name:'選擇小黑／小白'}]:[]),{id:'__chapter_card__',name:`章節開場卡｜${chapter.chapterCardTitle??chapter.title}`},...allNodes.filter(node=>node.id!==entryPicker?.id).map(node=>({id:node.id,name:label(node)}))]
