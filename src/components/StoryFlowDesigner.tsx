...
const extractFlowNode=(nodes:StoryFlowNode[],id:string):{nodes:StoryFlowNode[];node?:StoryFlowNode}=>{let found:StoryFlowNode|undefined;const next:StoryFlowNode[]=[];for(const item of nodes){if(item.id===id){found=item;continue}if(item.type==='branch'){const branches=item.branches.map(route=>{if(found)return route;const result=extractFlowNode(route.nodes,id);if(result.node)found=result.node;return{...route,nodes:result.nodes}});next.push({...item,branches})}else next.push(item)}return{nodes:next,node:found}}
const insertFlowNodeInRoute=(nodes:StoryFlowNode[],branchId:string,routeId:string,moving:StoryFlowNode):StoryFlowNode[]=>nodes.map(item=>item.type==='branch'?{...item,branches:item.branches.map(route=>item.id===branchId&&route.id===routeId?{...route,nodes:[...route.nodes,moving]}:{...route,nodes:insertFlowNodeInRoute(route.nodes,branchId,routeId,moving)})}:item)
const moveFlowNodeToRoute=(nodes:StoryFlowNode[],nodeId:string,branchId:string,routeId:string):StoryFlowNode[]=>{const source=flattenFlowNodes(nodes).find(node=>node.id===nodeId);if(!source||source.id===branchId||(source.type==='branch'&&flattenFlowNodes([source]).some(node=>node.id===branchId)))return nodes;const extracted=extractFlowNode(nodes,nodeId);return extracted.node?insertFlowNodeInRoute(extracted.nodes,branchId,routeId,extracted.node):nodes}
const insertFlowNodeAfter=(nodes:StoryFlowNode[],targetId:string,newNode:StoryFlowNode):{nodes:StoryFlowNode[];inserted:boolean}=>{const index=nodes.findIndex(node=>node.id===targetId);if(index>=0){const next=[...nodes];next.splice(index+1,0,newNode);return{nodes:next,inserted:true}}for(let i=0;i<nodes.length;i++){const item=nodes[i];if(item.type!=='branch')continue;for(let routeIndex=0;routeIndex<item.branches.length;routeIndex++){const result=insertFlowNodeAfter(item.branches[routeIndex].nodes,targetId,newNode);if(result.inserted){const branches=item.branches.map((route,index)=>index===routeIndex?{...route,nodes:result.nodes}:route);const next=[...nodes];next[i]={...item,branches};return{nodes:next,inserted:true}}}}return{nodes,inserted:false}}
...
export default function StoryFlowDesigner({ chapter, boardCharacters, onSave, onChapterChange, onClose }: Props) {
  ...
  const change = (next: StoryFlowNode[]) => { const linked=addAutomaticFlowLinks(next),valid=new Set(['__start__','__route_picker__','__world_map__','__chapter_card__',...flattenFlowNodes(linked).map(node=>node.id)]),graph=(chapter.flowGraphLinks??[]).filter(link=>valid.has(link.sourceId)&&valid.has(link.targetId)),card=(chapter.chapterCardNextLinks??[]).filter(link=>valid.has(link.targetId)),positions=Object.fromEntries(Object.entries(chapter.flowNodePositions??{}).filter(([id])=>valid.has(id))),disabled=(chapter.flowGraphDisabledLinks??[]).filter(key=>{const[source,target]=key.split('>');return valid.has(source)&&valid.has(target)});setFlow(linked);onChapterChange({flowGraphLinks:graph,chapterCardNextLinks:card,chapterCardNextNodeId:card[0]?.targetId,flowNodePositions:positions,flowGraphDisabledLinks:disabled});onSave(linked,rewards) }
...
function ChapterSettingsCard({chapter,rewards,onDuplicate,onChapterChange,onRewardsChange}:{chapter:StoryChapter;rewards:StoryRewards;onDuplicate:()=>void;onChapterChange:(patch:Partial<StoryChapter>)=>void;onRewardsChange:(rewards:StoryRewards)=>void}){
  ...
  return <section id="story-chapter-opening-settings" className="story-chapter-settings story-reward-node">
    ...
    <div className="story-chapter-settings-grid">
      ...
      {([['beforeBattleBgmStorageKey','戰鬥前音樂'],['battleBgmStorageKey','戰鬥中音樂'],['afterBattleBgmStorageKey','戰鬥後音樂']] as const).map(([key,label])=><label key={key}><span>{label}</span><select value={chapter[key]===undefined?'__inherit__':chapter[key]} onChange={e=>onChapterChange({[key]:e.target.value==='__inherit__'?undefined:e.target.value})}><option value="__inherit__">{key==='beforeBattleBgmStorageKey'?'沿用關卡音樂':'繼承全域 BGM'}</option><option value="">不播放音樂</option>{bgmTracks.map(track=><option key={track.storageKey} value={track.storageKey}>{track.name}</option>)}</select></label>)}
    </div>
...
function FlowGraphOverview({chapter,nodes,selectedNodeId,onChange,onChapterChange,onLocate,open,onOpenChange}:{chapter:StoryChapter;nodes:StoryFlowNode[];selectedNodeId?:string;onChange:(nodes:StoryFlowNode[])=>void;onChapterChange:(patch:Partial<StoryChapter>)=>void;onLocate:(id:string)=>void;open:boolean;onOpenChange:(open:boolean)=>void}){
  ...
  const[selectedLink,setSelectedLink]=useState<DrawnLink|null>(null)
  const enableGraphLink=(sourceId:string,targetId:string)=>{const key=`${sourceId}>${targetId}`,disabled=chapter.flowGraphDisabledLinks??[];if(disabled.includes(key))onChapterChange({flowGraphDisabledLinks:disabled.filter(item=>item!==key)})}
  const beginLink=(event:React.PointerEvent<HTMLDivElement>)=>{const element=event.target as HTMLElement,routeHandle=element.closest<HTMLElement>('[data-route-link-source]'),card=element.closest<HTMLElement>('[data-graph-node-id]');if(routeHandle){event.preventDefault();event.stopPropagation();setLinkSource(`__route_picker__::${routeHandle.dataset.routeLinkSource}`);event.currentTarget.setPointerCapture(event.pointerId);return}if(!card||card.dataset.graphNodeId==='__start__')return;const rect=card.getBoundingClientRect();if(event.clientX<rect.right-30)return;event.preventDefault();event.stopPropagation();setLinkSource(card.dataset.graphNodeId??null);event.currentTarget.setPointerCapture(event.pointerId)}
  const beginCardDrag=(event:PointerEvent)=>{const element=event.target as HTMLElement,card=element.closest<HTMLElement>('[data-graph-node-id]');if(!card||element.closest('button,input,select,[contenteditable="true"]'))return;const rect=card.getBoundingClientRect();if(card.dataset.graphNodeId!=='__start__'&&event.clientX>=rect.right-30)return;event.preventDefault();event.stopPropagation();const id=card.dataset.graphNodeId!,legacy=id==='__chapter_card__'?{x:chapter.chapterCardEditorX??0,y:chapter.chapterCardEditorY??0}:{x:0,y:0},origin=chapter.flowNodePositions?.[id]??legacy,startX=event.clientX,startY=event.clientY;card.classList.add('structure-dragging');const move=(next:PointerEvent)=>{next.preventDefault();onChapterChange({flowNodePositions:{...(chapter.flowNodePositions??{}),[id]:{x:origin.x+next.clientX-startX,y:origin.y+next.clientY-startY}}})},up=(next:PointerEvent)=>{card.classList.remove('structure-dragging');const route=document.elementsFromPoint(next.clientX,next.clientY).map(item=>item.closest<HTMLElement>('[data-flow-route-id][data-flow-branch-id]')).find(Boolean);if(route&&flattenFlowNodes(nodes).some(node=>node.id===id))onChange(moveFlowNodeToRoute(nodes,id,route.dataset.flowBranchId!,route.dataset.flowRouteId!));queueMicrotask(()=>void flushStoryChapters());window.removeEventListener('pointermove',move);window.removeEventListener('pointerup',up)};window.addEventListener('pointermove',move,{passive:false});window.addEventListener('pointerup',up)}
  const finishLink=(event:React.PointerEvent<HTMLDivElement>)=>{if(!linkSource)return;const target=document.elementFromPoint(event.clientX,event.clientY)?.closest<HTMLElement>('[data-graph-node-id]'),rect=target?.getBoundingClientRect();const targetId=target?.dataset.graphNodeId,realNodes=flattenFlowNodes(nodes),routeSource=linkSource.startsWith('__route_picker__::')?linkSource.split('::')[1]:undefined;if(targetId===linkSource){if(event.currentTarget.hasPointerCapture(event.pointerId))event.currentTarget.releasePointerCapture(event.pointerId);return}if(targetId&&targetId!=='__start__'&&rect&&event.clientX<=rect.left+36){if(routeSource&&entryPicker&&realNodes.some(node=>node.id===targetId)){onChange(moveFlowNodeToRoute(nodes,targetId,entryPicker.id,routeSource))}else{enableGraphLink(linkSource,targetId);if(linkSource==='__chapter_card__'&&realNodes.some(node=>node.id===targetId)){const links=chapter.chapterCardNextLinks??[];if(!links.some(link=>link.targetId===targetId))onChapterChange({chapterCardNextNodeId:chapter.chapterCardNextNodeId??targetId,chapterCardNextLinks:[...links,{id:uid('chapter_link'),targetId,label:`連線 ${links.length+1}`}]})}else if(realNodes.some(node=>node.id===linkSource&&node.type==='common')&&realNodes.some(node=>node.id===targetId))onChange(reconnectFlowNodes(nodes,linkSource,targetId));else{const links=chapter.flowGraphLinks??[];if(!links.some(link=>link.sourceId===linkSource&&link.targetId===targetId)){const defaultTargets=linkSource==='__world_map__'?[entryPicker?'__route_picker__':'__chapter_card__']:linkSource==='__chapter_card__'?[chapter.chapterCardNextNodeId??nodes.find(node=>node.id!==entryPicker?.id)?.id??entryPicker?.branches[0]?.nodes[0]?.id].filter(Boolean) as string[]:[],disabled=[...new Set([...(chapter.flowGraphDisabledLinks??[]),...defaultTargets.filter(id=>id!==targetId).map(id=>`${linkSource}>${id}`)])];onChapterChange({flowGraphLinks:[...links,{id:uid('graph_link'),sourceId:linkSource,targetId,label:`連線 ${links.filter(link=>link.sourceId===linkSource).length+1}`}],flowGraphDisabledLinks:disabled})}}}queueMicrotask(()=>void flushStoryChapters())}setLinkSource(null);if(event.currentTarget.hasPointerCapture(event.pointerId))event.currentTarget.releasePointerCapture(event.pointerId)}
  useEffect(()=>{const stage=stageRef.current;if(!stage)return;const down=(event:PointerEvent)=>{const element=event.target as HTMLElement,routeHandle=element.closest('[data-route-link-source]'),card=element.closest<HTMLElement>('[data-graph-node-id]'),rect=card?.getBoundingClientRect();if(routeHandle||card&&card.dataset.graphNodeId!=='__start__'&&rect&&event.clientX>=rect.right-30)beginLink(event as unknown as React.PointerEvent<HTMLDivElement>);else beginCardDrag(event)},up=(event:PointerEvent)=>finishLink(event as unknown as React.PointerEvent<HTMLDivElement>),cancel=()=>setLinkSource(null);stage.addEventListener('pointerdown',down);stage.addEventListener('pointerup',up);stage.addEventListener('pointercancel',cancel);return()=>{stage.removeEventListener('pointerdown',down);stage.removeEventListener('pointerup',up);stage.removeEventListener('pointercancel',cancel)}},[linkSource,nodes,chapter.flowNodePositions,chapter.chapterCardEditorX,chapter.chapterCardEditorY])
  ...
  useEffect(()=>{const stage=stageRef.current;stage?.querySelectorAll('.detail-selected').forEach(card=>card.classList.remove('detail-selected'));if(selectedNodeId)stage?.querySelector<HTMLElement>(`[data-graph-node-id="${CSS.escape(selectedNodeId)}"]`)?.classList.add('detail-selected')},[selectedNodeId,nodes])
  useEffect(()=>{const cards=[...stageRef.current?.querySelectorAll<HTMLElement>('[data-graph-node-id]')??[]];cards.forEach(card=>{const id=card.dataset.graphNodeId!,legacy=id==='__chapter_card__'?{x:chapter.chapterCardEditorX??0,y:chapter.chapterCardEditorY??0}:{x:0,y:0},position=chapter.flowNodePositions?.[id]??legacy;card.style.transform=`translate(${position.x}px,${position.y}px)`;card.classList.add('flow-draggable')})},[chapter.flowNodePositions,chapter.chapterCardEditorX,chapter.chapterCardEditorY,nodes])
  useEffect(()=>{const picker=nodes.find((node):node is Extract<StoryFlowNode,{type:'branch'}>=>node.type==='branch'&&!!node.chapterRouteSelect),card=stageRef.current?.querySelector<HTMLElement>('[data-graph-node-id="__route_picker__"]');if(!picker||!card)return;card.querySelector('.story-route-link-ports')?.remove();const ports=document.createElement('div');ports.className='story-route-link-ports';picker.branches.forEach((route,index)=>{const button=document.createElement('button'),label=document.createElement('span'),dot=document.createElement('i');button.type='button';button.dataset.routeLinkSource=route.id;button.dataset.routeIndex=String(index);button.title=`從「${route.label}」拖到目標節點左側藍點`;label.textContent=route.label;button.append(label,dot);ports.append(button)});const editButton=card.querySelector(':scope>button');card.insertBefore(ports,editButton);return()=>ports.remove()},[nodes])
  ...
  const nodeName=(id:string)=>{const system:Record<string,string>={__route_picker__:'選擇小黑／小白',__world_map__:'候選地圖',__chapter_card__:'章節開場卡'};const found=allNodes.find(node=>node.id===id);return system[id]??(found?label(found):id)}
  ...
  const removeLink=(link:DrawnLink)=>{
    ...
    if(source?.type==='common'){
      ...
      if(match){onChange(editFlowLink(nodes,source.id,match.id,{},true));return}
      if(source.nextNodeId===link.to){onChange(disconnectFlowNodes(nodes,source.id,link.to));return}
    }
    const key=`${link.from}>${link.to}`
    onChapterChange({flowGraphDisabledLinks:[...new Set([...(chapter.flowGraphDisabledLinks??[]),key])]})
  }
  const canRemoveLink=(_link:DrawnLink)=>true
  useLayoutEffect(()=>{
    const stage=stageRef.current;if(!stage)return
    const disabledLinks=new Set(chapter.flowGraphDisabledLinks??[])
...
function DialogueCard({ segment, boardCharacters, onChange }: { segment: StorySegment; boardCharacters: BoardCharacter[]; onChange: (segment: StorySegment) => void }) {
  ...
  const pose=segment.pose??'front',image = selectedPieceId ? ((pose==='awaken_front'?getUrlByKey(`cb_awaken_img_${selectedPieceId}`):pose==='awaken_side'?getUrlByKey(`cb_awaken_wide_img_${selectedPieceId}`):null)??getCharImg(selectedPieceId)??'') : (getUrlByKey(`cb_board_${segment.boardCharacter ?? 'black'}_${pose}`)??getUrlByKey(`cb_board_${segment.boardCharacter ?? 'black'}_${pose.endsWith('side')?'side':'front'}`)??'')
  return <div className="story-dialogue-editor">
    ...
    <div className="story-node-controls"><select value={segment.boardCharacter ?? ''} onChange={event => { const value = event.target.value; const pieceId = pieceIdFromRef(value); const character = pieceId ? characters.find(item => item.id === pieceId) : boardCharacters.find(item => item.id === value); patch({ boardCharacter: value || undefined, speaker: character?.name ?? segment.speaker, pose: 'front' }) }}><option value="">無大頭貼</option><optgroup label="看板角色">{boardCharacters.map(character => <option key={character.id} value={character.id}>{character.name}</option>)}</optgroup><optgroup label="所有棋子">{characters.map(character => <option key={character.id} value={`piece:${character.id}`}>{character.name}</option>)}</optgroup></select>
      ...
      <select value={segment.pose ?? 'front'} onChange={event => patch({ pose: event.target.value as StorySegment['pose'] })}><option value="front">正面</option><option value="side">側面</option><option value="awaken_front">覺醒正面</option><option value="awaken_side">覺醒側面</option></select></div>
  </div>
}
