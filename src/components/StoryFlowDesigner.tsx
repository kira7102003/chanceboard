...
function FlowGraphOverview({chapter,nodes,onChange,onChapterChange,onLocate,open,onOpenChange}:{chapter:StoryChapter;nodes:StoryFlowNode[];onChange:(nodes:StoryFlowNode[])=>void;onChapterChange:(patch:Partial<StoryChapter>)=>void;onLocate:(id:string)=>void;open:boolean;onOpenChange:(open:boolean)=>void}){
  ...
  const[manualSource,setManualSource]=useState('__route_picker__'),[manualTarget,setManualTarget]=useState('__world_map__'),[manualLabel,setManualLabel]=useState('下一步')
  ...
  const finishLink=(event:React.PointerEvent<HTMLDivElement>)=>{if(!linkSource)return;const target=document.elementFromPoint(event.clientX,event.clientY)?.closest<HTMLElement>('[data-graph-node-id]'),rect=target?.getBoundingClientRect();const targetId=target?.dataset.graphNodeId;if(targetId&&targetId!=='__start__'&&targetId!==linkSource&&rect&&event.clientX<=rect.left+36){if(linkSource==='__chapter_card__'){const links=chapter.chapterCardNextLinks??[];if(!links.some(link=>link.targetId===targetId))onChapterChange({chapterCardNextNodeId:chapter.chapterCardNextNodeId??targetId,chapterCardNextLinks:[...links,{id:uid('chapter_link'),targetId,label:`連線 ${links.length+1}`}]})}else if(flattenFlowNodes(nodes).some(node=>node.id===linkSource&&node.type==='common'))onChange(reconnectFlowNodes(nodes,linkSource,targetId));else{const links=chapter.flowGraphLinks??[];if(!links.some(link=>link.sourceId===linkSource&&link.targetId===targetId))onChapterChange({flowGraphLinks:[...links,{id:uid('graph_link'),sourceId:linkSource,targetId,label:`連線 ${links.filter(link=>link.sourceId===linkSource).length+1}`}]})}}setLinkSource(null);if(event.currentTarget.hasPointerCapture(event.pointerId))event.currentTarget.releasePointerCapture(event.pointerId)}
  ...
  const nodeName=(id:string)=>{const found=allNodes.find(node=>node.id===id);return found?label(found):id}
  const linkNodes=[...(entryPicker?[{id:'__route_picker__',name:'雙看板選線'}]:[]),{id:'__world_map__',name:'候選地圖'},{id:'__map_chapter__',name:`地圖關卡｜${chapter.mapNodeTitle??chapter.title}`},{id:'__chapter_card__',name:`章節開場卡｜${chapter.chapterCardTitle??chapter.title}`},...allNodes.filter(node=>node.id!==entryPicker?.id).map(node=>({id:node.id,name:label(node)}))]
  const createManualConnection=()=>{if(!manualSource||!manualTarget||manualSource===manualTarget)return;if(manualSource==='__chapter_card__'){const links=chapter.chapterCardNextLinks??[];if(!links.some(link=>link.targetId===manualTarget))onChapterChange({chapterCardNextNodeId:chapter.chapterCardNextNodeId??manualTarget,chapterCardNextLinks:[...links,{id:uid('chapter_link'),targetId:manualTarget,label:manualLabel||'下一步'}]});return}const common=allNodes.find(node=>node.id===manualSource&&node.type==='common');if(common){onChange(reconnectFlowNodes(nodes,manualSource,manualTarget,manualLabel||'下一步'));return}const links=chapter.flowGraphLinks??[];if(!links.some(link=>link.sourceId===manualSource&&link.targetId===manualTarget))onChapterChange({flowGraphLinks:[...links,{id:uid('graph_link'),sourceId:manualSource,targetId:manualTarget,label:manualLabel||'下一步'}]})}
  const linkToolbar=<section className="story-graph-link-toolbar"><b>自己串接節點</b><label><span>從哪個節點</span><select value={manualSource} onChange={e=>setManualSource(e.target.value)}>{linkNodes.map(node=><option key={node.id} value={node.id}>{node.name}</option>)}</select></label><i>→</i><label><span>接到哪個節點</span><select value={manualTarget} onChange={e=>setManualTarget(e.target.value)}>{linkNodes.filter(node=>node.id!==manualSource).map(node=><option key={node.id} value={node.id}>{node.name}</option>)}</select></label><label><span>連線名稱</span><input value={manualLabel} onChange={e=>setManualLabel(e.target.value)}/></label><button disabled={manualSource===manualTarget} onClick={createManualConnection}>＋ 建立連線</button><small>也可以從卡片右側黃點拖到另一張卡左側藍點</small></section>
  useLayoutEffect(()=>{
