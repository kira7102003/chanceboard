...
const detachFlowNode=(nodes:StoryFlowNode[],id:string):{nodes:StoryFlowNode[];node?:StoryFlowNode}=>{let found:StoryFlowNode|undefined;const walk=(items:StoryFlowNode[]):StoryFlowNode[]=>items.flatMap<StoryFlowNode>(item=>{if(item.id===id){found=item;return[]}if(item.type==='branch')return[{...item,branches:item.branches.map(route=>({...route,nodes:walk(route.nodes)}))}];return[item]});return{nodes:walk(nodes),node:found}}
const insertFlowNodeAfter=(nodes:StoryFlowNode[],sourceId:string,node:StoryFlowNode):{nodes:StoryFlowNode[];inserted:boolean}=>{let inserted=false;const walk=(items:StoryFlowNode[]):StoryFlowNode[]=>{const next:StoryFlowNode[]=[];for(const item of items){const value=item.type==='branch'?{...item,branches:item.branches.map(route=>({...route,nodes:walk(route.nodes)}))}:item;next.push(value);if(item.id===sourceId){next.push(node);inserted=true}}return next};return{nodes:walk(nodes),inserted}};
const reconnectFlowNodes=(nodes:StoryFlowNode[],sourceId:string,targetId:string)=>{if(sourceId===targetId)return nodes;const detached=detachFlowNode(nodes,targetId);if(!detached.node)return nodes;const inserted=insertFlowNodeAfter(detached.nodes,sourceId,detached.node);return inserted.inserted?inserted.nodes:nodes}
...
function FlowGraphOverview({chapter,nodes,onChange,onChapterChange,onLocate,open,onOpenChange}:{chapter:StoryChapter;nodes:StoryFlowNode[];onChange:(nodes:StoryFlowNode[])=>void;onChapterChange:(patch:Partial<StoryChapter>)=>void;onLocate:(id:string)=>void;open:boolean;onOpenChange:(open:boolean)=>void}){
  ...
  const beginLink=(event:React.PointerEvent<HTMLDivElement>)=>{const card=(event.target as HTMLElement).closest<HTMLElement>('[data-graph-node-id]');if(!card||card.classList.contains('branch')||card.dataset.graphNodeId?.startsWith('__'))return;const rect=card.getBoundingClientRect();if(event.clientX<rect.right-26)return;event.preventDefault();setLinkSource(card.dataset.graphNodeId??null);event.currentTarget.setPointerCapture(event.pointerId)}
  const finishLink=(event:React.PointerEvent<HTMLDivElement>)=>{if(!linkSource)return;const target=document.elementFromPoint(event.clientX,event.clientY)?.closest<HTMLElement>('[data-graph-node-id]');const targetId=target?.dataset.graphNodeId;if(targetId&&!targetId.startsWith('__'))onChange(reconnectFlowNodes(nodes,linkSource,targetId));setLinkSource(null);if(event.currentTarget.hasPointerCapture(event.pointerId))event.currentTarget.releasePointerCapture(event.pointerId)}
  const dragChapterCard=(event:React.PointerEvent<HTMLElement>)=>{if((event.target as HTMLElement).closest('button'))return;event.preventDefault();event.stopPropagation();const startX=event.clientX,startY=event.clientY,originX=chapter.chapterCardEditorX??0,originY=chapter.chapterCardEditorY??0;const move=(next:PointerEvent)=>onChapterChange({chapterCardEditorX:originX+next.clientX-startX,chapterCardEditorY:originY+next.clientY-startY});const end=()=>{window.removeEventListener('pointermove',move);window.removeEventListener('pointerup',end)};window.addEventListener('pointermove',move);window.addEventListener('pointerup',end)}
  ...
  },[nodes,collapseVersion,chapter.chapterCardEditorX,chapter.chapterCardEditorY])
