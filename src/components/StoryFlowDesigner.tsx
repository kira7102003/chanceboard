...
function FlowGraphOverview({nodes,onChange,onLocate,open,onOpenChange}:{nodes:StoryFlowNode[];onChange:(nodes:StoryFlowNode[])=>void;onLocate:(id:string)=>void;open:boolean;onOpenChange:(open:boolean)=>void}){
  ...
  useLayoutEffect(()=>{
    ...
    const draw=()=>{const base=stage.getBoundingClientRect();setLinks(edges.map(edge=>{const from=stage.querySelector<HTMLElement>(`[data-graph-node-id="${CSS.escape(edge.from)}"]`)?.getBoundingClientRect(),to=stage.querySelector<HTMLElement>(`[data-graph-node-id="${CSS.escape(edge.to)}"]`)?.getBoundingClientRect();if(!from||!to)return edge;const x1=from.right-base.left+stage.scrollLeft,y1=from.top+from.height/2-base.top+stage.scrollTop,x2=to.left-base.left+stage.scrollLeft,y2=to.top+to.height/2-base.top+stage.scrollTop,curve=Math.max(45,Math.abs(x2-x1)*.42);return{...edge,path:`M ${x1} ${y1} C ${x1+curve} ${y1}, ${x2-curve} ${y2}, ${x2} ${y2}`}}))}
