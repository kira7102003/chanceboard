...
function FlowGraphOverview({chapter,nodes,selectedNodeId,onChange,onChapterChange,onLocate,open,onOpenChange}:{chapter:StoryChapter;nodes:StoryFlowNode[];selectedNodeId?:string;onChange:(nodes:StoryFlowNode[])=>void;onChapterChange:(patch:Partial<StoryChapter>)=>void;onLocate:(id:string)=>void;open:boolean;onOpenChange:(open:boolean)=>void}){
  ...
  useEffect(()=>{const cards=[...stageRef.current?.querySelectorAll<HTMLElement>('[data-graph-node-id]')??[]],cleanups:CallableFunction[]=[];cards.forEach(card=>{const id=card.dataset.graphNodeId!,legacy=id==='__chapter_card__'?{x:chapter.chapterCardEditorX??0,y:chapter.chapterCardEditorY??0}:{x:0,y:0},position=chapter.flowNodePositions?.[id]??legacy;card.style.transform=`translate(${position.x}px,${position.y}px)`;card.classList.add('flow-draggable');const down=(event:PointerEvent)=>{if((event.target as HTMLElement).closest('button,input,select,[contenteditable="true"]')||event.clientX>=card.getBoundingClientRect().right-30)return;event.preventDefault();event.stopPropagation();const startX=event.clientX,startY=event.clientY,origin=chapter.flowNodePositions?.[id]??legacy;card.classList.add('structure-dragging');const move=(next:PointerEvent)=>onChapterChange({flowNodePositions:{...(chapter.flowNodePositions??{}),[id]:{x:origin.x+next.clientX-startX,y:origin.y+next.clientY-startY}}}),up=(next:PointerEvent)=>{card.classList.remove('structure-dragging');const route=document.elementsFromPoint(next.clientX,next.clientY).map(element=>element.closest<HTMLElement>('[data-flow-route-id][data-flow-branch-id]')).find(Boolean);if(route&&flattenFlowNodes(nodes).some(node=>node.id===id))onChange(moveFlowNodeToRoute(nodes,id,route.dataset.flowBranchId!,route.dataset.flowRouteId!));queueMicrotask(()=>void flushStoryChapters());window.removeEventListener('pointermove',move);window.removeEventListener('pointerup',up)};window.addEventListener('pointermove',move);window.addEventListener('pointerup',up)};card.addEventListener('pointerdown',down);cleanups.push(()=>card.removeEventListener('pointerdown',down))});return()=>cleanups.forEach(cleanup=>cleanup())},[chapter.flowNodePositions,chapter.chapterCardEditorX,chapter.chapterCardEditorY,nodes])
