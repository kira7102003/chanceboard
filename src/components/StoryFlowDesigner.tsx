...
function FlowGraphOverview({chapter,nodes,onChange,onChapterChange,onLocate,open,onOpenChange}:{chapter:StoryChapter;nodes:StoryFlowNode[];onChange:(nodes:StoryFlowNode[])=>void;onChapterChange:(patch:Partial<StoryChapter>)=>void;onLocate:(id:string)=>void;open:boolean;onOpenChange:(open:boolean)=>void}){
  ...
  const finishLink=(event:React.PointerEvent<HTMLDivElement>)=>{if(!linkSource)return;const target=document.elementFromPoint(event.clientX,event.clientY)?.closest<HTMLElement>('[data-graph-node-id]');const targetId=target?.dataset.graphNodeId;if(targetId&&targetId!=='__start__'){if(linkSource==='__chapter_card__')onChapterChange({chapterCardNextNodeId:targetId});else onChange(reconnectFlowNodes(nodes,linkSource,targetId))}setLinkSource(null);if(event.currentTarget.hasPointerCapture(event.pointerId))event.currentTarget.releasePointerCapture(event.pointerId)}
  ...
  useLayoutEffect(()=>{
    ...
    edges.push({from:'__start__',to:'__chapter_card__',color:'#ffd45e'});const chapterTarget=chapter.chapterCardNextNodeId??nodes[0]?.id;if(chapterTarget)edges.push({from:'__chapter_card__',to:chapterTarget,color:chapter.chapterCardNextNodeId?'#f3c84d':'#ffd45e',label:chapter.chapterCardNextNodeId?'自訂':'自動'});walk(nodes)
  ...
  },[nodes,collapseVersion,chapter.chapterCardEditorX,chapter.chapterCardEditorY,chapter.chapterCardNextNodeId])
