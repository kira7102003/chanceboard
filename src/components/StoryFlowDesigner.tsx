...
function FlowGraphOverview({chapter,nodes,onChange,onChapterChange,onLocate,open,onOpenChange}:{chapter:StoryChapter;nodes:StoryFlowNode[];onChange:(nodes:StoryFlowNode[])=>void;onChapterChange:(patch:Partial<StoryChapter>)=>void;onLocate:(id:string)=>void;open:boolean;onOpenChange:(open:boolean)=>void}){
  ...
  const finishLink=(event:React.PointerEvent<HTMLDivElement>)=>{if(!linkSource)return;const target=document.elementFromPoint(event.clientX,event.clientY)?.closest<HTMLElement>('[data-graph-node-id]'),rect=target?.getBoundingClientRect();const targetId=target?.dataset.graphNodeId;if(targetId&&targetId!=='__start__'&&rect&&event.clientX<=rect.left+32){if(linkSource==='__chapter_card__')onChapterChange({chapterCardNextNodeId:targetId});else onChange(reconnectFlowNodes(nodes,linkSource,targetId))}setLinkSource(null);if(event.currentTarget.hasPointerCapture(event.pointerId))event.currentTarget.releasePointerCapture(event.pointerId)}
