...
const reconnectFlowNodes=(nodes:StoryFlowNode[],sourceId:string,targetId:string):StoryFlowNode[]=>nodes.map(node=>node.id===sourceId&&node.type==='common'?{...node,nextNodeId:node.nextNodeId??targetId,nextLinkMode:'manual',nextLinks:(node.nextLinks??[]).some(link=>link.targetId===targetId)?node.nextLinks:[...(node.nextLinks??[]),{id:uid('link'),targetId,label:`連線 ${(node.nextLinks?.length??0)+1}`}]}:node.type==='branch'?{...node,branches:node.branches.map(route=>({...route,nodes:reconnectFlowNodes(route.nodes,sourceId,targetId)}))}:node)
...
function FlowGraphOverview({chapter,nodes,onChange,onChapterChange,onLocate,open,onOpenChange}:{chapter:StoryChapter;nodes:StoryFlowNode[];onChange:(nodes:StoryFlowNode[])=>void;onChapterChange:(patch:Partial<StoryChapter>)=>void;onLocate:(id:string)=>void;open:boolean;onOpenChange:(open:boolean)=>void}){
  ...
  const beginLink=(event:React.PointerEvent<HTMLDivElement>)=>{const card=(event.target as HTMLElement).closest<HTMLElement>('[data-graph-node-id]');if(!card||card.classList.contains('branch')||card.dataset.graphNodeId==='__start__')return;const rect=card.getBoundingClientRect();if(event.clientX<rect.right-26)return;event.preventDefault();event.stopPropagation();setLinkSource(card.dataset.graphNodeId??null);event.currentTarget.setPointerCapture(event.pointerId)}
  const finishLink=(event:React.PointerEvent<HTMLDivElement>)=>{if(!linkSource)return;const target=document.elementFromPoint(event.clientX,event.clientY)?.closest<HTMLElement>('[data-graph-node-id]'),rect=target?.getBoundingClientRect();const targetId=target?.dataset.graphNodeId;if(targetId&&targetId!=='__start__'&&targetId!==linkSource&&rect&&event.clientX<=rect.left+32){if(linkSource==='__chapter_card__'){const links=chapter.chapterCardNextLinks??[];if(!links.some(link=>link.targetId===targetId))onChapterChange({chapterCardNextNodeId:chapter.chapterCardNextNodeId??targetId,chapterCardNextLinks:[...links,{id:uid('chapter_link'),targetId,label:`連線 ${links.length+1}`}]})}else onChange(reconnectFlowNodes(nodes,linkSource,targetId))}setLinkSource(null);if(event.currentTarget.hasPointerCapture(event.pointerId))event.currentTarget.releasePointerCapture(event.pointerId)}
  ...
  useEffect(()=>{const cards=[...stageRef.current?.querySelectorAll<HTMLElement>('[data-system-card]')??[]],cleanups:CallableFunction[]=[];cards.forEach(card=>{const id=card.dataset.graphNodeId!,position=chapter.flowNodePositions?.[id]??{x:0,y:0};card.style.transform=`translate(${position.x}px,${position.y}px)`;const down=(event:PointerEvent)=>{if((event.target as HTMLElement).closest('button,input,select')||event.clientX>=card.getBoundingClientRect().right-24)return;event.preventDefault();const startX=event.clientX,startY=event.clientY,origin=chapter.flowNodePositions?.[id]??{x:0,y:0};const move=(next:PointerEvent)=>onChapterChange({flowNodePositions:{...(chapter.flowNodePositions??{}),[id]:{x:origin.x+next.clientX-startX,y:origin.y+next.clientY-startY}}}),up=()=>{window.removeEventListener('pointermove',move);window.removeEventListener('pointerup',up)};window.addEventListener('pointermove',move);window.addEventListener('pointerup',up)};card.addEventListener('pointerdown',down);cleanups.push(()=>card.removeEventListener('pointerdown',down))});return()=>cleanups.forEach(cleanup=>cleanup())},[chapter.flowNodePositions])
  ...
  },[nodes,collapseVersion,chapter.chapterCardEditorX,chapter.chapterCardEditorY,chapter.chapterCardNextNodeId,chapter.chapterCardNextLinks])
