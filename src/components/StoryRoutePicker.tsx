...
export default function StoryRoutePicker({chapter,node,onChoose,onBack}:{chapter:StoryChapter;node:BranchNode;onChoose:(route:BranchNode['branches'][number])=>void;onBack:()=>void}){
 ...
 return <div className="story-route-picker"><section>
  ...
  <div className="story-route-picker-grid">{node.branches.map(route=>{const coverKey=route.coverKey||`cb_story_${route.id}`,image=getUrlByKey(coverKey)||(/^(https?:|data:|\/)/.test(coverKey)?coverKey:null);return <button className={`story-route-cover${image?'':' empty'}`} key={route.id} onClick={()=>onChoose(route)} style={image?{backgroundImage:`linear-gradient(transparent 45%,#020309f5),url(${image})`}:undefined}>{!image&&<strong>尚未設定「{route.label}」選擇 CG</strong>}<span><b>{route.label}</b><p>{route.description||`${route.label}路線，體驗不同的選擇與結局。`}</p><small>{progress(route)}</small></span></button>})}</div>
